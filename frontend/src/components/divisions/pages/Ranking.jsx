import Container from "../../common/Container"
import GoldImg from '../../../assets/img/divisionGold.png'
import SilverLockedImg from '../../../assets/img/divisionSilverLocked.png'
import BronzeLockedImg from '../../../assets/img/divisionBronzeLocked.png'
import UserImg from '../../../assets/img/user.png'

const usersList = [
    { id: 1, name: 'Usuario 1', score: 1369 },
    { id: 2, name: 'Usuario 2', score: 1369 },
    { id: 3, name: 'Usuario 3', score: 1369 },
    { id: 4, name: 'Usuario 4', score: 1369 },
    { id: 5, name: 'Usuario 5', score: 1369 },
    { id: 6, name: 'Usuario 6', score: 1369 },
    { id: 7, name: 'Usuario 7', score: 1369 },
    { id: 8, name: 'Usuario 8', score: 1369 },
    { id: 9, name: 'Usuario 9', score: 1369 },
    { id: 10, name: 'Usuario 10', score: 1369 },
]


const Ranking = () => {
    return (
        <Container>
            <section className="mt-1">
                <div className="gap-5 flex justify-around items-center">
                    <img className="w-[64.89px] h-[86.52px]" src={BronzeLockedImg} alt="Bronze" />
                    <img className="w-[64.89px] h-[86.52px]" src={SilverLockedImg} alt="Silver" />
                    <img className="w-[64.89px] h-[86.52px]" src={GoldImg} alt="Gold" />
                </div>
                <h2 className="mt-9 text-center text-black text-regular-18 font-semibold capitalize">División Oro</h2>
            </section>

            <section className="shadow-soft mt-3 rounded-2xl">
                <div className="py-2 px-6 flex justify-between text-regular text-secondary">
                    <div>
                        <span>#</span>
                        <span className="ms-6">Nombre de usuario</span>
                    </div>
                    <div>Puntos</div>
                </div>

                {/* LISTADO USUARIOS */}
                {
                    usersList.map(item => (
                        <div key={item.id} className={`${item.id == 1 ? 'bg-ranking-user' : ''} py-2 ps-5 pe-6 border-t border-primary flex justify-between items-center text-regular-16 text-secondary`}>
                            <div className="flex">
                                <span className="w-[25px] text-blue text-[24px] text-center">{item.id}</span>
                                <div className="flex justify-center items-center ms-4">
                                    <img className="w-[42px] h-[42px]" src={UserImg} alt="user" />
                                    <span className="ms-3">{item.name}</span>
                                </div>
                            </div>
                            <span className="text-secondary text-regular-14">{item.score}</span>
                        </div>
                    ))
                }
            </section>
        </Container>
    )
}
export default Ranking