...
const Dropdown = ({ options, selectedOption, setSelectedOption }) => {
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            ...
            {isOpen && (
                <ul className="absolute bg-[#EFEFF0] mt-1 w-[184.81px] border border-gray-300 rounded-b-md shadow-lg z-10">
                    {options.map((option, index) => (
                        <li key={index} onClick={() => handleSelect(option)} className="px-4 py-2 text-sm hover:bg-gray-100 cursor-pointer">
                            {option}
                        </li>
                    ))}
                </ul>
